...
test.describe.serial("account lifecycle", () => {
  ...
  test("registration, duplicate email, honeypot, and verification", async ({ page, context }) => {
    ...
    // (old one invalidated).
    const dupRes = await context.request.post(`${FRONTEND_URL}/api/auth/register`, {
    ...
    const botRes = await context.request.post(`${FRONTEND_URL}/api/auth/register`, {
    ...
    const weakRes = await context.request.post(`${FRONTEND_URL}/api/auth/register`, {
  ...
  test("cross-user isolation holds for freshly-registered accounts (RLS regression)", async ({ page, context }) => {
    ...
    const userBConversations = await (await context.request.get(`${FRONTEND_URL}/api/conversations`)).json();
    ...
    await ctxAIsolationCheck.request.post(`${FRONTEND_URL}/api/auth/login`, { data: userA });
    const userAConversations = await (await ctxAIsolationCheck.request.get(`${FRONTEND_URL}/api/conversations`)).json();
