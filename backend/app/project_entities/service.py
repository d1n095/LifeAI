...
def record_interpretation_proposal(
    ...
    *,
    ...
    try:
        ...
        savepoint.rollback()
        if constraint_name != "uq_interpretation_proposals_idem":
            raise
        existing = db.execute(
            select(InterpretationProposal).where(
                InterpretationProposal.owner_id == owner_id,
                InterpretationProposal.idempotency_key == idempotency_key,
            )
        ).scalar_one_or_none()
        if existing is None:
            raise
