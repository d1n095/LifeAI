...
def settle_job_cost(
    ...
    *,
    ...
    # settle_provider_spend_call() performs the canonical mutation through a SECURITY DEFINER
    # SQL function. If this Session already holds the reservation row in its identity map,
    # SQLAlchemy can otherwise hand us the pre-settle object. Refresh before mirroring the
    # canonical actual into the Program-local envelope so stale ORM identity cannot hide real
    # settled spend.
    db.refresh(event)
