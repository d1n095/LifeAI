...
class SourceImportBatchStatus(str, enum.Enum):
    ...
    docs/LIFE_SOURCE_FOUNDATION_BOOTSTRAP.md §P."""
...
class SourceImportBatch(Base):
    """A corpus intake session (docs/LIFE_SOURCE_FOUNDATION_BOOTSTRAP.md §E) -- e.g. "Founder
...
class SourceImportBatchFailure(Base):
    """One row per source that failed during a batch (docs/LIFE_SOURCE_FOUNDATION_BOOTSTRAP.md
    ...
    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    owner_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("users.id"), index=True)
    batch_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("source_import_batches.id"), index=True)
