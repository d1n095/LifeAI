...
_MAINAI_EXECUTION_TABLES = (
    ...
    # Migration 0071 (Prediction vs Outcome Learning)
    "prediction_records",
    # Migration 0070 (Hot/Warm/Cold Memory)
    "memory_tier_states",
    # Migration 0071 (Self-Improvement ROI)
    ...
    # content, deletion only through the SECURITY DEFINER erasure path.
    "work_candidates",
    # Migration 0057 (Life Execution Authorization Envelope): execution_scope_proposals and
    # execution_authorization_envelopes play the SAME structural role -- mutable rows whose
...
def apply_mainai_execution_privileges(engine: Engine, *, require_complete: bool = True) -> None:
    ...
    with engine.begin() as conn:
        ...
        conn.execute(text("REVOKE DELETE, TRUNCATE, REFERENCES, TRIGGER ON founder_intent_bindings FROM mainai_app"))
        conn.execute(text("REVOKE UPDATE, DELETE, TRUNCATE, REFERENCES, TRIGGER ON founder_intent_corrections FROM mainai_app"))
        conn.execute(text("REVOKE DELETE, TRUNCATE, REFERENCES, TRIGGER ON structured_claims FROM mainai_app"))
        ...
        for table, allowed in (
            ...
            *((table, frozenset({"SELECT", "INSERT"})) for table in (
            ...
            ("founder_intent_bindings", frozenset({"SELECT", "INSERT", "UPDATE"})),
            ("founder_intent_corrections", frozenset({"SELECT", "INSERT"})),
            ("structured_claims", frozenset({"SELECT", "INSERT", "UPDATE"})),
