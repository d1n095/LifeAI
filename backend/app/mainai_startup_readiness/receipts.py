...
def verify_kill_switch_schema(db: Session | None) -> ReadinessCheck:
    if db is None:
        return ReadinessCheck(
            "kill_switch_health",
            CheckStatus.unknown,
            "no db session for durable stop-control check",
        )
    try:
        db.execute(text("SELECT scope, active, sequence FROM mainai_stop_state LIMIT 1")).first()
        return ReadinessCheck(
            "kill_switch_health",
            CheckStatus.healthy,
            "durable mainai_stop_state readable",
            evidence={"durable": True, "process_global_only": False},
        )
    except Exception as exc:
        return ReadinessCheck(
            "kill_switch_health",
            CheckStatus.unhealthy,
            f"stop controls unavailable: {exc}",
        )
