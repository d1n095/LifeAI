...
INTEGRATION NOTE (superseding the original note below, kept for history): this migration was
originally authored on `claude/mainai-job-runtime-foundation` as `revision = "0025"`,
`down_revision = "0018"` — that branch's own base at the time. Both PR #31 (S1A provenance,
merged as `0019`-`0024`) and PR #35 (durable backfill-run reporting, merged as its own
`0025_memory_source_backfill_runs`) landed on the base branch afterward, so the real current
head is `0025_memory_source_backfill_runs`, not `0018`. During integration
(`claude/mainai-job-runtime-integration`) this file was renamed from `0025_mainai_jobs.py` to
`0026_mainai_jobs.py` and renumbered exactly as this docstring's original note said it would
need to be: `revision` is now `0026`, `down_revision` now points at `0025` (i.e.
`0025_memory_source_backfill_runs`, PR #35's real head), so the chain is
`0025_memory_source_backfill_runs` -> `0026_mainai_jobs` -> `0027_mainai_job_integrity`
(likewise renumbered from `0026`). No SQL in `upgrade()`/`downgrade()` below was changed —
only the revision identity and this docstring.

ORIGINAL NOTE (historical — the collision it predicted is the one just resolved above): this
branch (claude/mainai-job-runtime-foundation) is built from the base branch
