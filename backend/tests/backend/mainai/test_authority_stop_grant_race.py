...
def test_record_boot_blocked_uses_single_blocking_identity(superuser_db):
    ...
    gseq = query_stop_status(superuser_db, owner_id=a.id)["global"]["sequence"]
    record_boot_blocked(superuser_db, owner_id=a.id, reason="boot_saw_stop")
    superuser_db.flush()
    row = superuser_db.execute(
        sa_text(
            "SELECT scope, sequence, owner_id FROM mainai_stop_events "
            "WHERE event_kind='boot_blocked' ORDER BY created_at DESC LIMIT 1"
        )
    ).one()
    assert row[0] == "global"
    assert int(row[1]) == int(gseq)
