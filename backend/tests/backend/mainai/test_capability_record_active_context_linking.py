...
def test_capability_record_edges_expose_its_verification_evidence(superuser_db):
    ...
    evidence = record_evidence(
        superuser_db, owner_id=owner.id, execution_id=execution.id, evidence_kind="deterministic_check", review_kind="deterministic_tool",
        deterministic=True, payload={"passed": True, "capability_key": capability_key}, source_type="ci_log", source_ref="cap-probe", idempotency_key="cap-edge-ev",
    )
    capability = record_capability_observation(
