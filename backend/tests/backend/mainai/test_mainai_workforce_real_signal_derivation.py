...
def _capability_evidence(superuser_db, *, owner_id, capability_key: str):
    goal = MainAIGoal(owner_id=owner_id, title="capability proof", original_instruction="prove", created_by="test", completed_at=None)
    ...
    superuser_db.flush()
    task = MainAITask(
        owner_id=owner_id, goal_id=goal.id, plan_id=plan.id, task_type="repo_edit", description="capability proof",
        status="pending", risk_level="low",
    )
    superuser_db.add(task)
    superuser_db.flush()
    execution = record_execution(superuser_db, owner_id=owner_id, task_id=task.id, idempotency_key=f"wfsig-exec-{uuid.uuid4()}", provider="internal")
    evidence = record_evidence(
        superuser_db, owner_id=owner_id, execution_id=execution.id, evidence_kind="test_run_result",
        payload={"passed": True, "capability_key": capability_key}, source_type="pytest",
        source_ref=f"tests/{capability_key}.py::test_verified", idempotency_key=f"wfsig-ev-{uuid.uuid4()}", deterministic=True,
    )
    return evidence
...
def test_sparse_mastery_history_falls_through_to_capability_reality(superuser_db, owner_id):
    ...
    evidence = _capability_evidence(superuser_db, owner_id=owner_id, capability_key="rare_capability")
    record_capability_observation(
        superuser_db, owner_id=owner_id, capability_key="rare_capability", domain="backend",
        status="verified_available", confidence=0.75, success=True, verification_evidence_id=evidence.id,
    )
    superuser_db.commit()
