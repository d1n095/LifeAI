...
| MainAI Cognitive Efficiency + Systemic Debugging + Situational Awareness + Information Lifecycle + Repo/Backup Intelligence (Founder Anti-Repetition + Cross-Agent Duplication Control + Compatibility Graph + Change Impact + Fragmentation/Defragmentation/Deduplication + Context Packaging + Self-Optimizing Context + Recovery Checkpoints + Repo/Backup Intelligence) | ny, `app.mainai_cognitive_ops` | **Klar, redo för oberoende granskning** — se `docs/mainai_v2/MAINAI_COGNITIVE_OPS_RECONCILIATION.md` + `docs/mainai_v2/HANDOFF_CLAUDE_COGNITIVE_OPS.md`. Byggd ovanpå Research/Truth/Advisory-kandidaten utan att ändra dess befintliga funktioner (endast tre nya read-only-hjälpfunktioner tillagda additivt i `research_ledger.py`). En additiv migration (0074: `mainai_ops_founder_communications`, append-only via samma återanvända 0038-trigger, + GIN-index på `mainai_research_evidence_links.provenance`). `repo_backup_intelligence.py` gör RIKTIG, read-only git-introspektion (fast argv, aldrig `shell=True`, aldrig en mutating subcommand — strukturellt bevisat). Stänger Research-rondens tre P1:or: cross-investigation auto-reopen, durable investigation graph-beslut (behåller JSONB + GIN-index + real validering, inte en ny tabell), och Resource Intelligence → Provider Economics-bridge (UNKNOWN != ZERO, MISSING != FREE). Hittade och fixade två riktiga buggar under bygget (self-referential append-only-trigger-brott, `.strip()` som korrupterade `git status --porcelain`-parsning) samt en konkret verklig upptäckt: branchen låg 5 commits före origin (endast lokalt) vid byggtillfället — rapporterat till grundaren, inte auto-pushat. 58/58 egna tester + 1 ny research_ledger-test gröna; bred regression se nedan. FRYST kandidat, SHA `7015ce4d66c3df291b72ac23d15180fad1ed2f44` — nästa rad forsätter från denna SHA på egen branch. |
| MainAI Coverage & Omission Intelligence + Dynamic Workforce Orchestration + Capability Learning/External-Independence (Maturity-ladder-återanvändning + Traceability Chain + Automatic Discovery + Layered Matching + Real Signal Derivation + Wait-vs-Assign + Continue-vs-Handoff + Reservation + Capability Mastery Ledger + Promotion/Demotion + Teacher Value + External Dependence Reduction + Founder Output) | **ny egen branch** `claude/mainai-v2-coverage-workforce-capability` (grenad från `claude/mainai-v2-sovereign`s frysta SHA `7015ce4`, EJ mergad till sovereign ännu), worktree `.../claude-mainai-v2-coverage-workforce` | **Klar, redo för oberoende granskning (två ronder)** — se `docs/mainai_v2/MAINAI_COVERAGE_WORKFORCE_CAPABILITY_RECONCILIATION.md` + `docs/mainai_v2/HANDOFF_CLAUDE_COVERAGE_WORKFORCE_CAPABILITY.md`. Rond 1 (`4a57d89`): två nya paket, `app.mainai_coverage` (ingen migration — återanvänder `mainai_vision.types.MaturityState`/`completion.py`s dynamic-denominator-mekanik VERBATIM) och `app.mainai_workforce` (additiv migration 0075: durabel Capability Mastery Ledger). 48/48 tester, 452/452 bred regression. Rond 2 (uppföljning på grundarens egen P1-granskning): stänger de två kvarvarande P1:orna — **A** riktig, read-only automatisk källingestion (`source_adapters.py`: handoffs, reconciliation-docs, branch registry, git-logg; conversation-history explicit `SourceAvailability(available=False)` — UNKNOWN != ABSENT, aldrig fabricerad), **B** lagerad matchningsstrategi (`matching.py`: EXACT_IDENTITY → NORMALIZED_LEXICAL → ALIAS → STRUCTURED_LINK → GRAPH_RELATIONSHIP → KEYWORD_OVERLAP → SEMANTIC_SIMILARITY, aldrig tyst maskerad som semantisk förståelse; SEMANTIC_SIMILARITY permanent otillgänglig — ingen extern AI-dependency införd), **C** riktig signalhärledning (`signal_derivation.py`/`real_state_decisions.py`: kompetens/rework/quota/context-relevans härledd från Mastery Ledger → capability_reality → resource_intelligence, aldrig en parallell omimplementation av de redan testade rena beslutsfunktionerna). Två riktiga buggar hittade och fixade under Rond 2 (kort numerisk-token-precision i keyword-matchning; `deduplicate_claims()` fick INTE slå ihop två olika claims enbart på KEYWORD_OVERLAP — "SIMILAR WORDING == SAME REQUIREMENT"). **E**: testsvit-skalbarhets-flakighet (322 fel i EN körning av ~505 tester, 0 fel i 3 direkta omkörningar + `pg_stat_activity`-mätning som utesluter connection-exhaustion) evidensklassad och dokumenterad separat (`docs/mainai_v2/KNOWN_ISSUE_TEST_SUITE_COMBINED_RUN_FLAKINESS.md`) — INTE fixad, utanför scope, bekräftat orelaterad till all kod i denna eller tidigare V2-ronder. 89/89 egna tester gröna (Rond 1+2 tillsammans). |
