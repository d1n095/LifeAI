...
| MainAI Coverage & Omission Intelligence + Dynamic Workforce Orchestration + Capability Learning/External-Independence (Maturity-ladder-återanvändning + Traceability Chain + Historical Omission Discovery + Dynamic Completion Denominator-komposition + Wait-vs-Assign + Continue-vs-Handoff + Reservation + Capability Mastery Ledger + Promotion/Demotion + Teacher Value + External Dependence Reduction + Founder Output) | **ny egen branch** `claude/mainai-v2-coverage-workforce-capability` (grenad från `claude/mainai-v2-sovereign`s frysta SHA `7015ce4`, EJ mergad till sovereign ännu), worktree `.../claude-mainai-v2-coverage-workforce` | **Klar, redo för oberoende granskning** — se `docs/mainai_v2/MAINAI_COVERAGE_WORKFORCE_CAPABILITY_RECONCILIATION.md` + `docs/mainai_v2/HANDOFF_CLAUDE_COVERAGE_WORKFORCE_CAPABILITY.md`. Två nya paket: `app.mainai_coverage` (ingen migration — återanvänder `mainai_vision.types.MaturityState`/`completion.py`s dynamic-denominator-mekanik VERBATIM istället för att bygga en konkurrerande stege; ny ortogonal `CoverageDisposition` för SUPERSEDED/REJECTED/BLOCKED/UNACCOUNTED_FOR) och `app.mainai_workforce` (en additiv migration 0075: durabel Capability Mastery Ledger, append-only events via samma 0038-trigger). Komponerar RIKTIGT med `app.agent_coordination.runtime_view` (stänger en P1 från förra rondens handoff) och `app.mainai_vision.gap_generator`s befintliga staging-pipeline (aldrig promotion). `next_best_resource_allocation()` (resource_intelligence) medvetet INTE sammanslagen med denna rondens `workforce_scheduler` — olika frågor, komponeras separat. 48/48 egna tester gröna. Bred regression: 452/452 grönt för denna rondens eget scope + alla tidigare paket; ett OREL­ATERAT, pre-existerande test-skalningsfynd upptäckt (se registrets not nedan) — INTE orsakat av denna kod, isolerat och verifierat separat. |
