...
each based directly on integration tip (no interdependency between them); #241 is based on
**#239's own branch** (not integration tip) because it fixes the one gap #239's own PR
description explicitly flagged as out of its scope — see #241's row below.
...
three rows will be redundant with what's already on `claude/det-kommer-mer-879lcm`; the #241
row is only here.)*
...
`safe_composed_run.py` signature-mismatch bug described in #241's row above.
