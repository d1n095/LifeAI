...
|---|---|---|---|
| `codex/universal-personal-recall` | — | **Canonical outbox/disclosure hardening isolated — production activation blocked; do not merge/wire** | Default-off unregistered handlers, transactional PostgreSQL outbox triggers, monotonic source generations, disclosure-time content equality, owner-bound version fencing, bounded reconciliation, persistent retry leases/dead-letter state, incremental canonical reads, FTS candidate helper and crash recovery proof. Based on `ea179c2`; no #245 changes, Claude branch imports, chat/UI wiring or production registrations. Reviewed AEAD/key hierarchy, retention/compaction, external dependency scheduling and independent attack review remain pending. See `docs/PERSONAL_RECALL_CANONICAL_OUTBOX.md`. |
