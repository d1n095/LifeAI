...
| **V4** | YES | OPEN, not started | 6 remaining `chat_with_fallback()` callers (rows 7-13 minus row 1, now closed) — zero redaction found in any of them. The `owner_id` kwarg exists on `chat_with_fallback()` now; wiring the remaining callers is mechanical, not a redesign. `mainai_execution/execution_job.py` is currently Cursor-owned (open PR #168) -- do not touch until it merges/closes. |
