...
| **V2** | YES — closed | **CLOSED** (`embed_with_policy()` wired into all 4 embedding call sites) | Document-chunk embedding (`rag/ingest.py`), transcript-chunk embedding (`rag/media_import.py`), and now query embedding (`rag/retrieve.py`'s `retrieve_context()`, `routers/library.py`'s hybrid search) are all gated. `retrieve_context()`'s `owner_id` was already a required parameter -- no identity-propagation gap to solve. Both query-embedding call sites preserve their existing graceful-degradation shape on denial (chat: ungrounded reply, still completes; library search: `semantic_search_available=False`, ILIKE text-match channel unaffected) -- an `EgressDeniedError` is never treated as a provider outage and never triggers a silent retry against a different provider (embedding has no fallback chain in MainAI 0.1 at all). `routers/knowledge.py`'s `/api/knowledge/search` and `routers/workbench.py`'s `retrieve_context()` call have no try/except around it at all today (any provider failure already 500s there) -- gating doesn't regress that, but doesn't add graceful degradation either; unchanged behavior, not a new gap. |
...
| **V4** | YES — mostly closed | **PARTIAL** | Audited all 6 remaining `chat_with_fallback()` callers. **Closed** (owner_id structurally available, no invented identity): `routers/workbench.py` (`user.id` from `require_founder`), `mainai_execution/planner.py`'s `propose_plan_via_ai()` (`goal.owner_id`), `jobs/handlers/corpus_review.py` (`document.uploaded_by` — routed to a per-document `document_skipped` outcome, never a whole-job failure, since it now sits ahead of the generic `except Exception` catch-all that would otherwise misclassify a content-policy denial as an unexpected system error). **Still open, genuinely no owner to wire** (identified, not invented): `mainai_execution/lesson_conflicts.py`'s `detect_conflict()` and `agent_orchestration.py`'s `dispatch_task()`/`review_task()` — both operate on `EngineeringLesson`/`AgentTask` rows that are explicitly documented in their own model docstrings as "founder-wide project/system knowledge, not per-owner personal data" (no `owner_id` column exists on either table). MainAI 0.1 is single-founder-only today, so there IS in practice only one possible owner, but hardcoding a founder lookup here would be inventing an identity binding the data model itself doesn't have — the real fix is a founder-level decision on whether/how these tables should carry provenance, not a gate workaround. `mainai_execution/execution_job.py` is currently Cursor-owned (open PR #168) -- not audited, do not touch until it merges/closes. |
...
| **V6** | Foundational, blocks the founder's own audit requirement | **PARTIAL** (migration 0062, PR #170) | `provider_disclosure_events` ledger exists and is written by every gated call site (V1, V2, V3 all closed now). Structurally CANNOT yet answer "what has provider X ever received about project/document Y" -- see **V8** below, a real gap this table has, not just an unclosed call site. |
...
| **V8** | YES — the ledger's own core audit promise | OPEN, not started, design needed | `provider_disclosure_events` has `task_id`/`goal_id`/`job_id` (MainAI-execution-scoped, always NULL for every RAG/chat call site — chat.py/ingest.py/media_import.py/retrieve.py/library.py never pass them) but **no `document_id`/`project_id`/`conversation_id` linkage at all**. The table can truthfully answer "did provider X ever receive ANYTHING from owner Y, and when" (owner_id + provider + decision + timestamp), but **cannot** answer the founder's own stated requirement "what has provider X ever received about project/document Y" -- there is structurally no way to join a ledger row back to the specific Document/Conversation/project it disclosed content from. Closing this needs a durable source-reference linkage (e.g. a `source_refs jsonb` column or a link table `provider_disclosure_event_sources(event_id, document_id)`), populated at each gated call site from the same `hits`/`document_id` data those sites already have in scope (chat.py's `hits`, ingest.py's `document.id`, library.py's search results) — never raw content, only reference ids, matching the ledger's own existing hash-only-content discipline. Not implemented here per the founder's own "identify the gap, don't build speculative UI/schema yet" instruction — this row exists so the gap is tracked, not silently discovered again later. |

## Log/trace leakage audit (2026-08-27, founder-requested)

Audited every path that can raise or handle `EgressDeniedError` for content leaking through
logging rather than the provider call itself. Finding: **no leakage found**, by construction,
not by luck:
- `app/egress_policy/service.py` performs **zero logging of any kind** -- the entire audit
  trail is the `provider_disclosure_events` ledger (hashes only, see migration 0062), never a
  log line.
- `EgressDeniedError.__str__()` always renders `f"egress denied: {reason}"` where `reason` is
  one of a small number of fixed, static strings this module itself writes (e.g. "payload
  contains a NEVER_EGRESS-marked field...") -- never interpolated from the payload, never
  `repr()` of the denied content. This is true even where `EgressDeniedError` propagates
  *uncaught* (`routers/knowledge.py`'s `/api/knowledge/search`, `routers/workbench.py`'s
  `retrieve_context()` call currently have no try/except around it at all -- pre-existing,
  identical to how any other provider failure already 500s there): whatever ends up in a
  server-side exception log is still just that same safe, static string.
- Every explicit `except EgressDeniedError` site added across V1/V2 (`chat.py`, `ingest.py`,
  `media_import.py`, `library.py`) surfaces only `str(exc)`, matching the exact same safety
  discipline `classify_provider_exception()` already established for ordinary provider
  failures (never `str()` of a raw provider exception, which can embed request URLs/keys).
