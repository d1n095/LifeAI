...
| **B4** | **YES — over-broad write surface** | IN PROGRESS (parallel PR) | Cursor | `production_entry` sets every `WorkBinding.allowed_paths` to the **full** envelope ceiling (documented in its own module note). Accepted plans do NOT narrow task scope today. Fix: derive binding paths/capabilities as `intersection(envelope, paths/caps cited in Safe-Planner-validated plan)` — plan may only narrow, never widen; never treat planner output as founder authority. Leave final `production_entry` wire until after Claude B2. |
| **B5** | Partial | OPEN | Cursor | Supervisor E2E proves **task** completion. Goal rollup / next-task selection still needs a production_entry tick once spend is wired. |
